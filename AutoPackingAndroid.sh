...
# 【配置上传到蒲公英相关信息】(可选)
#__PGYER_U_KEY="蒲公英的User Key "cla
...
cd ../
__CURRENT_PATH=`pwd`
echo "${__LINE_BREAK_LEFT} 当前目录：：：：=${__CURRENT_PATH} ${__LINE_BREAK_RIGHT}"
# 检查是否打包成功 build 下面的apk目录是否存在
if test -d "${__CURRENT_PATH}/build/app/outputs/apk" ; then
...
echo "${__LINE_BREAK_LEFT} 归档目录：：：：=${__EXPORT_APK_PATH} ${__LINE_BREAK_RIGHT}"
...
mv ${__CURRENT_PATH}/build/app/outputs/apk/* ${__EXPORT_APK_PATH}/
